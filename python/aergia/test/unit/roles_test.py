...
from aergia._roles import RoleNotFound, prompt, load, parse, render
...
@pytest.fixture
def role_factory():
    def factory(name, path, content, meta):
        role = path / f"{name}.role"
        role.write_text(content)
        metadata = path / f"{name}.role.metadata"
...
@pytest.fixture
def comedian_role(tmp_path, role_factory):
    name = "comedian"
    definition = cleandoc(
        """
        You are a comedian and telling the audiance a joke about:
        {{ role.input }}
        """
    )
    metadata = {}
    yield role_factory(name=name, path=tmp_path, content=definition, meta=metadata)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "role:arg1:arg2:arg3",
            Expected(role="role", args=["arg1", "arg2", "arg3"], kwargs={}),
        ),
...
def test_execute_kw_arg():
    expected = "Please translate the text below to german:\n" "Hello how are you?"
    role_spec = "babelfish:lang=german"
    input = "Hello how are you?"
    actual, model = prompt(role_spec, input=input)
    assert actual == expected
...
def test_load_throws_exception_if_role_is_not_found():
    role = "This_role_should_not_exist"
    with pytest.raises(RoleNotFound):
        load(role)


def test_load_picks_up_custom_roles(comedian_role):
    name, role, metadata = comedian_role
    path = role.parent

    role_template, role_metadata = load(name, paths=[path])

    expected = str(role)
    actual = role_template.filename
    assert actual == expected

    expected = json.loads(metadata.read_text())
    actual = role_metadata
    assert actual == expected
