...
@pytest.mark.parametrize(
    ...
    [
        (
            Setting(
                name="model", type=str, default="gpt-4o", description="Model to use"
            ),
            ("MODEL", "gpt-4o"),
        ),
        (
...
@pytest.mark.parametrize(
    ...
    [
        (
            Setting(name="limit", type=int, default=5, description="Fetch limit"),
            {"LIMIT": "2", "A": "B"},
            2,
        ),
        (
        ...
        (
            Setting(
                name="stream", type=bool, default="on", description="Enable streaming"
            ),
            {"STREAM": "TRUE", "A": "B"},
            True,
        ),
...
def test_setting_from_env(setting, env, expected):
    actual = from_env(name=env_key(setting), type=setting.type, env=env)
    assert actual == expected
