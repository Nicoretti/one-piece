...
impl Clock {
    fn hours(&self) -> i32 {
        (self.minutes / 60) as i32 % 24
    }

    fn minutes(&self) -> i32 {
        (self.minutes % 60) as i32
    }

    pub fn new(hours: i32, minutes: i32) -> Self {
        let hours = if hours.is_negative() {
            24 + (hours % 24)
        } else {
            hours
        };
        let minutes: i64 = ((hours * 60) + minutes) as i64;
        let minutes = if minutes.is_negative() {
            let h = 24 + ((minutes / 60) % 24);
            let m = minutes % 60;
            h * 60 + m
        } else {
            minutes
